...
//Llama al método initComponents de la propia clase
...
//Inicializa un objeto "RenderWindow", al cuál se le asigna  un nombre
//y la dimensiones  que tendrá la ventana.
//Inicializa un objeto "Image" la cuál exportará un textura
//y la va a almacenar
//Después se declara un variable Unit8 la cuál será 
//un puntero que almacenará la primera posición (primer pixel) de la imagen 
//este mismo será asignado como ícono a la ventana creada anteriomente
//Realiza un ciclo el cual se matendrá abierto mientras la ventana no sea cerrada y
//dentro de este mismo ciclo se instanciará un objeto tipo evento el cuál
//será capturado dentro de otro ciclo el estará verificando los eventos creados por el usuario
//Finalmente el método dibuja los atributos de la clase
...
void View::start() {
	...
	i.loadFromFile("iconos/icons8-calculator-50.png");
...
//Recibe por parámetro un objeto tipo "Model"
//el cual será asignado como model al objeto instanciado
//de la esta clase
...
//Recibe por parámetro un objeto tipo "Controller"
//el cual será asignado como controller al objeto instanciado
//de la esta clase
void View::setController(Controller* controller) { this->controller = controller; }


//Carga las imágenes del proyecto y las guarda en "sprites", estos 
//mismos serán dibujados más adelante. A cada "sprite" se le asigna un 
//tamaño y las coordenadas donde se van a dibujar.
//Carga fuentes de la carpeta y las almacena en variables tipo "font",
//las cuales van a ser utilizadas cómo decoración en la calculadora
//y para la impresión de datos relevantes en la misma, a cada una se le
//asigna su tamaño y sus coordenadas
void View::initComponents() {

	if (!texture.loadFromFile("iconos/fondo.jpg"))
		return;

	sprite.setTexture(texture);
	sprite.setTextureRect(sf::IntRect(0, 0, 490, 500));

	if (!icon1.loadFromFile("iconos/icons8-1-50.png"))
		return;

	numero1.setTexture(icon1);
	numero1.setPosition(55, 440);

	if (!icon2.loadFromFile("iconos/icons8-2-50.png"))
		return;

	numero2.setTexture(icon2);
	numero2.setPosition(165, 440);

	if (!icon3.loadFromFile("iconos/icons8-3-50.png"))
		return;

	numero3.setTexture(icon3);
	numero3.setPosition(275, 440);

	if (!icon_plus_minus.loadFromFile("iconos/icons8-plus-and-minus-50.png"))
		return;

	plus_minus.setTexture(icon_plus_minus);
	plus_minus.setPosition(385, 440);

	if (!icon4.loadFromFile("iconos/icons8-4-50.png"))
		return;

	numero4.setTexture(icon4);
	numero4.setPosition(55, 400);

	if (!icon5.loadFromFile("iconos/icons8-5-50.png"))
		return;

	numero5.setTexture(icon5);
	numero5.setPosition(165, 400);

	if (!icon6.loadFromFile("iconos/icons8-6-50.png"))
		return;

	numero6.setTexture(icon6);
	numero6.setPosition(275, 400);

	if (!icon_plus.loadFromFile("iconos/icons8-add-new-50.png"))
		return;

	plus.setTexture(icon_plus);
	plus.setPosition(385, 400);

	if (!icon7.loadFromFile("iconos/icons8-7-50.png"))
		return;

	numero7.setTexture(icon7);
	numero7.setPosition(55, 360);

	if (!icon8.loadFromFile("iconos/icons8-8-50.png"))
		return;

	numero8.setTexture(icon8);
	numero8.setPosition(165, 360);

	if (!icon9.loadFromFile("iconos/icons8-9-50.png"))
		return;

	numero9.setTexture(icon9);
	numero9.setPosition(275, 360);

	if (!icon_minus.loadFromFile("iconos/icons8-reduce-50.png"))
		return;

	minus.setTexture(icon_minus);
	minus.setPosition(385, 360);

	if (!icon0.loadFromFile("iconos/icons8-0-50.png"))
		return;

	numero0.setTexture(icon0);
	numero0.setPosition(55, 320);

	if (!icon_division.loadFromFile("iconos/icons8-division-50.png"))
		return;

	division.setTexture(icon_division);
	division.setPosition(165, 320);

	if (!icon_multiply.loadFromFile("iconos/icons8-multiplication-50.png"))
		return;

	multiply.setTexture(icon_multiply);
	multiply.setPosition(275, 320);

	if (!icon_clear.loadFromFile("iconos/icons8-clear-symbol-50.png"))
		return;

	clear.setTexture(icon_clear);
	clear.setPosition(385, 320);

	if (!icon_exponent.loadFromFile("iconos/icons8-exponent-200.png"))
		return;
	...
	if (!icon_bracket_open.loadFromFile("iconos/icons8-brackets-open.png"))
		return;
	...
	if (!icon_bracket_close.loadFromFile("iconos/icons8-brackets-close.png"))
		return;
	...
	if (!icon_trash.loadFromFile("iconos/icons8-empty-trash-50.png"))
		return;
	...
	if (!texture1.loadFromFile("iconos/fondo_boton.jpg"))
		return;
	...
	font1.loadFromFile("fuentes/OpenSans-SemiboldItalic.ttf");
...
//Verifica si todas las llamadas necesarias están correctas.
//Reliza las llamadas correspondientes al model para obtener los datos 
//y visualizarlos gráficamente.
//Seguidamente se utiliza la controladora para realizar los cálculos necesarios
//y obtener los nuevos resultados.
